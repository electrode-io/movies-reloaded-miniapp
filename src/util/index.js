...
const getImageSize = (fieldName) => (configuration, minWidth) => {
  ...
  const sizes = configuration.images[fieldName].filter(
    size => size &&
      size.startsWith('w') &&
      Number.parseInt(size.substring(1)) >= minWidth * WIDTH_MULTIPLIER
  )
...
const interpolatable = [
  'color', 'backgroundColor', 'fontSize', 'left', 'right', 'top', 'bottom',
...
export const interpolateStyles = (interpolater, from, to) => {
  ...
  interpolatable.forEach(prop => {
    if (
      (from.hasOwnProperty(prop) ||
      to.hasOwnProperty(prop)) &&
      from[prop] !== to[prop]
    ) {
      ...
      if (!fromHasProp || !toHasProp) {
        if (process.env.NODE_ENV === 'development') {
          console.warn(`Cannot interpolate property "${prop}" from ${from[prop]} to ${to[prop]}.\n\nMissing property in style object`, fromHasProp ? to : from)
        }
      } else if (
        (from.hasOwnProperty(prop) ||
        to.hasOwnProperty(prop)) &&
