...
import {interpolateStyles, oneOption, selectOption} from '../../util';
...
export default class ButtonBase extends Component {
  ...
  handlePressIn(...params) {
    ...
    this.setState(
      ...
      () => {
        Animated.timing(this.state.pressAnimator, {
  ...
  render() {
    ...
    if (!oneOption([primary, secondary, primaryMini, secondaryMini])) {
      console.warn(
        'ButtonBase can only accept one of: [primary, secondary, primaryMini, secondaryMini]',
      );
    } else {
      variant =
        selectOption([
    ...
    const iconComponent = icon && (
      <Animated.Image
        source={iconSource}
        style={[imageStyle.cleaned, imageStyle.interpolated]}
      />
    );
