...
export default class MovieDetails extends Component {
  ...
  static getDynamicTitle(jsonProps) {
    return (
      jsonProps && jsonProps.details && `Cast of ${jsonProps.details.title}`
    );
  }
      ...
      configuration && details && details.credits ? (
        details.credits.cast.map((cast, i) => (
          <CastMember
            key={`${i}_${cast.id}`}
            cast={cast}
            imageBase={`${configuration.images.base_url}${getCastImageSize(
              configuration,
              50,
            )}`}
            style={!i && {marginTop: 0}}
    ...
    return (
      <View style={styles.container}>
        ...
        <ScrollView style={{flex: 1}}>
          <View style={styles.content}>{castList}</View>
        </ScrollView>
      </View>
    );
  }
