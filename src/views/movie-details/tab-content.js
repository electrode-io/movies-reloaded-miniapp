    ...
    const overlayAnimator = index => {
      if (selectedIndex === index) {
        ...
        return previousIndex > selectedIndex
          ? Animated.multiply(
              -1,
              Animated.subtract(selectedIndex, tabAnimation),
            )
          : Animated.multiply(
              -1,
              Animated.subtract(tabAnimation, selectedIndex),
            );
      }
            ...
            <React.Fragment key={`Tab_${i}`}>
              <View
                ...
                <ScrollView style={{flex: 1}}>
                  <View
                    style={[
                      styles.tabBarPageContent,
                      tab.fixedContent ? {paddingBottom: 70} : {},
                    ]}>
                    {tab.content}
                  </View>
                </ScrollView>
