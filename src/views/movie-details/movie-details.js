...
import { getBackgroundImageSize, findGenres, getPosterImageSize, getCastImageSize } from '../../util'
...
export default class MovieDetails extends Component {
  ...
  loadMovieDetails () {
    ...
    this.setState({ loading: true, error: false, errorMessage: null }, () => {
      if (movie) {
        api.movieDetails({ uri: movie.id }).then(res => {
          if (res.status_code) {
            this.setState({
              loading: false,
              error: true,
  ...
  getPosterUri = (state, props) => {
    ...
    return `${configuration.images.base_url}${getPosterImageSize(configuration, viewport.width * 0.5)}${movie.poster_path}`
  }

  getPosterDisplaySize = (posterSize, viewportSize) => {
    const ratio = Math.min(viewportSize.height / posterSize.height, viewportSize.width / posterSize.width)
    return {
  ...
  render () {
    ...
    if (configuration && !loading && viewport && movie) {
      const backgroundBase = `${configuration.images.base_url}${getBackgroundImageSize(configuration, viewport.width)}`
      backgroundUri = `${backgroundBase}${movie.backdrop_path}`
    }

    const renderedBackground = Platform.OS === 'android' ? (
      <React.Fragment>
        <AnimatedBackgroundImage uri={backgroundUri} opacity={animation.interpolate({ inputRange: [0, 1], outputRange: [0, 0.5] })} />
      </React.Fragment>
    ) : (
    ...
    if (posterUri && posterSize) {
      const posterDisplaySize = this.getPosterDisplaySize(posterSize, { height: viewport.height * 0.5, width: viewport.width * 0.33 })
      renderedPoster = (
        <View style={{ width: posterDisplaySize.width }}>
          <Image resizeMode='contain' resizeMethod='resize' style={posterDisplaySize} source={{ uri: posterUri }} />
        </View>
      )
    }
    ...
    const tabs = [
      ...
      },
      {
        ...
        content: configuration && details && details.credits ? (
          details.credits.cast.filter((t, i) => i < 5).map((cast, i) =>
            <CastMember
              key={`${i}_${cast.id}`}
              cast={cast}
        ...
        fixedContent: (
          <View style={styles.overlay}>
            <PrimaryButton
              icon={'chevronRight'}
              iconRight
              onPress={() => this.navigateInternal('Cast', { details, configuration, backgroundUri })}
            >
              Full Cast
            </PrimaryButton>
          </View>
    ...
    const pageContent = !loading && posterSize ? (
      <React.Fragment>
        <View style={{ flex: 1, flexDirection: 'row' }}>
          <View style={{ alignSelf: 'flex-end', padding: 16, flex: 1 }}>
            <View style={{ flexDirection: 'row' }}>
              {renderedPoster}
              <View style={{ flex: 1, flexBasis: 0, flexGrow: 1, paddingLeft: 16, alignSelf: 'flex-end' }}>
                <Percentage size={45} value={movie.vote_average * 10}>
                  <Text style={styles.movieRatingText}>{movie.vote_average}</Text>
                </Percentage>
                <Text style={styles.movieTitle}>{movie.title}</Text>
                {details.tagline ? <Text style={styles.movieTagline}>{details.tagline}</Text> : <React.Fragment />}
                <View style={[styles.row, { flexWrap: 'wrap' }]}>
                  {genres && genres.map(genre => <Badge key={genre}>{genre}</Badge>)}
                </View>
              </View>
            </View>
          </View>
        </View>
        <View style={{ flex: 1, position: 'relative' }} onLayout={this.setLayout('tabViewport', ['width', 'height'])}>
          <TabBar tabs={tabs.map(tab => tab.title)} onTabChange={index => this.startTabAnimation(index)} />
          <TabContent
            viewport={viewport}
